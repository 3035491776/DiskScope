...
DiskScope 是 Windows 磁盘空间诊断工具。当前状态为 **V0.1 / M5 空间建议**。C: 仅可经固定范围和显式确认进行元数据分析；**NO CLEANUP EXECUTION YET**。
...
本阶段可显式分析固定 C:\ 范围，也保留 fixture 与项目工作区；D:\ 整卷、任意路径和网络路径仍拒绝。空间建议不依赖 ScanManager 内存，可在重启后使用 SQLite 最新已完成快照。没有清理、删除、移动、文件修改或缓存清理。扫描器不打开文件内容。安全边界由后端执行。
