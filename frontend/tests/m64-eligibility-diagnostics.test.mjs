...
test('summary explains zero eligibility, reason semantics, coverage, and both rule versions', () => {
  ...
  assert.match(page, /不是执行授权/)
  assert.match(page, /不是可释放空间/)
...
test('candidate explanations cover nested, recent, extension, and eligible future states', () => {
  assert.match(policyReasonTitle('EXECUTION_RISK_BLOCKED'), /风险等级/)
  ...
  assert.equal(eligibilityDecisionLabel('eligible_for_recycle'), '可进入准备处理')
  assert.equal(eligibilityDecisionLabel('ineligible'), '当前策略不允许处理')
...
test('blocked candidates remain explanatory and cannot expose an execution action', () => {
  ...
  assert.match(page, /不会自动准备或执行/)
  assert.match(page, /即使符合条件，也必须在准备处理时重新核对当前文件/)
})
