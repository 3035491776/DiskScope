...
test('coverage is human-readable while technical codes remain available', () => {
  assert.equal(coverageReasons.ACCESS_DENIED.label, '有些位置没有访问权限')
  assert.equal(coverageReasons.REPARSE_POINT_SKIPPED.label, '已跳过系统链接或重定向位置')
...
test('long paths shrink but sizes, risk and actions remain visible', () => {
  ...
  assert.match(recommendations, /为什么不能处理？/)
  assert.match(large, /class="size-cell"/)
  assert.match(large, /:title="absolutePath\(file.relative_path\)"/)
})
