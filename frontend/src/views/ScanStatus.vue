...
<template>
  ...
  <template v-else>
    <section class="panel status-detail"><div class="panel-heading"><div><p class="eyebrow">CURRENT TASK</p><h2>{{ targetName(store.scan.scope_key === 'system_drive_c' ? 'c_drive' : store.scan.scope_key === 'current_user_temp' ? 'user_temp' : store.scan.scope_key === 'project_workspace' ? 'project' : 'fixture') }}</h2></div><span class="status-pill" :class="store.scan.state">{{ completionLabel(store.scan) }}</span></div><p v-if="['system_drive_c', 'current_user_temp'].includes(store.scan.scope_key)" class="inline-note">Low-Impact Scan · Metadata Only · 单扫描任务。只读分析可随时取消，不读取正文，也不会删除文件。</p><ScanProgress :scan="store.scan" />
      ...
      <p v-if="store.scan.state === 'failed'" class="inline-error" role="alert">扫描失败：{{ store.scan.error_message || '请展开技术详情查看错误信息。' }}</p>
      ...
      <p v-if="store.scan.state === 'completed' && store.scan.snapshot_status === 'failed'" class="inline-error" role="alert">扫描已完成，但历史快照保存失败。当前扫描结果仍可查看。</p>
      ...
      <details class="coverage-technical"><summary>技术详情</summary><p>任务编号 <span class="mono">{{ store.scan.scan_id }}</span></p><p v-if="store.scan.error_code">错误代码 <span class="mono">{{ store.scan.error_code }}</span></p><p v-if="store.scan.snapshot_error_code">快照错误代码 <span class="mono">{{ store.scan.snapshot_error_code }}</span></p></details>
    </section>
    <div class="two-column">
      ...
      <section class="panel"><div class="panel-heading"><div><p class="eyebrow">RESOURCE USAGE</p><h2>扫描资源</h2></div><span class="subtle-label">当前任务资源概览</span></div>
